...
static int check_atomic(struct bpf_verifier_env *env, int insn_idx, struct bpf_insn *insn)
{
	...
	/* Older architecture JITs only implement legacy XADD. */
	if (insn->imm != BPF_ADD && env->prog->jit_requested &&
	    !bpf_jit_supports_atomics()) {
		verbose(env, "JIT does not support extended atomics\n");
		return -EOPNOTSUPP;
	}
					 ...
					 const union bpf_attr *attr)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (!aux[adj_idx].zext_dst) {
			 ...
			 *       BPF_STX + SRC_OP, so it is safe to pass NULL here.
			 */
